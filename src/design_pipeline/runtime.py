...
constraints:
  ...
  - "entity_id and workflow_id are INTERNAL METADATA ONLY, used purely to group screens in the review workspace's own navigation -- they are never user-facing product copy. NEVER render the raw entity_id or workflow_id string as visible text anywhere in a screen's HTML (not in <title>, headings, breadcrumbs, labels, or anywhere else). A screen titled for the ia_annual_plan entity reads as \\"Annual Plan Register\\", never \\"Annual Plan Register (ia_annual_plan)\\" or similar. This still applies when entity_id is an ordinary English word or acronym rather than a snake_case slug -- an entity named \\"Procedure\\" or \\"QAIP\\" reads as \\"Procedure Form\\" or \\"QAIP Register\\", never \\"Procedure Form (Procedure)\\" or \\"QAIP Register (QAIP)\\": do not add the entity_id as a parenthetical type-badge/tag next to the title, even though it looks like a harmless category label. Write natural, human-facing titles and labels as if entity_id/workflow_id did not exist."
